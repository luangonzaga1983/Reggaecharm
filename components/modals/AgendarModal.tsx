...
export default function AgendarModal({ usuario, stats, barbeiros, storeConfig, onClose, onSuccess }: Props) {
  ...
  useEffect(() => {
    if (barbeiroId && unidadeId && data) {
      fetch(`/api/agendamentos?action=horarios&barbeiro_id=${encodeURIComponent(barbeiroId)}&data=${encodeURIComponent(data)}`)
        .then(r => r.json()).then(d => { setOcupados(d.ocupados || []); setBloqueados(d.bloqueados || []); setReservados(d.reservados || []); setFolgaDia(!!d.folga_dia_todo); })
        .catch(() => { setOcupados([]); setBloqueados([]); setReservados([]); setFolgaDia(false); });
    }
  ...
  async function confirmar() {
    ...
    try {
      ...
      if (!res.ok) { setError(d.error || 'Algo deu errado. Tente de novo.'); return; }
      const novoId: string = d.agendamento?.id;
      if (!novoId) { setError('Algo deu errado ao criar o agendamento. Tente de novo.'); return; }
      setAgId(novoId);

      // Pago por crédito (vale-corte): já nasce grátis/confirmado, sem PIX.
      if (d.agendamento?.pago || d.agendamento?.pagamento_metodo === 'credito') { setStep('sucesso'); return; }
      ...
      if (!r2.ok) { setError(p.error || 'Não foi possível gerar o PIX. Tente de novo.'); setStep('pagamento'); return; }
      setPix({
            ...
            {data && (
              <div className="anim-in">
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 6 }}>
                  {horarios.map(h => {
                    const blocked = bloqueados.includes(h);    // almoço/folga → indisponível, SEM fila
                    const busy    = !blocked && ocupados.includes(h); // agendamento real → fila-ável
                    ...
                    return (
                      <button
                        key={h}
                        disabled={blocked || filaCheia || loading}
                        onClick={() => blocked ? undefined : busy ? reservarFila(h) : (setHorario(h), setStep('termos'))}
                        title={blocked ? 'Indisponível (almoço/folga)' : podeFila ? 'Ocupado — entrar na fila' : filaCheia ? 'Ocupado, fila cheia' : ''}
                        style={{
                          padding: '9px 4px',
                          borderRadius: 8,
                          ...
                          cursor: (blocked || filaCheia) ? 'not-allowed' : 'pointer',
                          border: podeFila ? '1px dashed var(--warning)' : '1px solid',
                          background: (busy || blocked) ? 'transparent' : sel ? 'var(--accent)' : 'var(--surface2)',
                          color: blocked ? 'var(--text-faint)' : filaCheia ? 'var(--text-faint)' : podeFila ? 'var(--warning)' : sel ? 'var(--accent-contrast)' : 'var(--text)',
                      ...
                      >
                        {blocked ? h : filaCheia ? `${h} · fila` : podeFila ? `${h} · fila?` : h}
                      </button>
                    );
                  })}
                </div>
