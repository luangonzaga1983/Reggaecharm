...
export default function ReagendarModal({ agendamento, onClose, onDone }: Props) {
  ...
  useEffect(() => {
    ...
    fetch(`/api/agendamentos?action=horarios&barbeiro_id=${encodeURIComponent(agendamento.barbeiro_id)}&data=${encodeURIComponent(data)}`)
      .then(r => r.json())
      .then(d => { setSlots(d.horarios || []); setOcupados(d.ocupados || []); setBloqueados(d.bloqueados || []); setReservados(d.reservados || []); setFolgaDia(!!d.folga_dia_todo); })
      .catch(() => { setSlots([]); setOcupados([]); setBloqueados([]); setReservados([]); setFolgaDia(false); });
    setHorario('');
  ...
  async function confirmar() {
    ...
    try {
      ...
      if (!res.ok) { setError(d.error || 'Não foi possível reagendar. Tente de novo.'); return; }
      onDone(); onClose();
    ...
    <div className="modal-back" onClick={e => e.target === e.currentTarget && onClose()}>
      <div className="modal-box" style={{ maxWidth: 460 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 6, marginBottom: 16 }}>
          {slots.map(h => {
            // Reagendar não tem fila: agendamento ocupado E almoço/folga = indisponível.
            const busy = ocupados.includes(h) || bloqueados.includes(h);
            const sel = horario === h;
            return (
