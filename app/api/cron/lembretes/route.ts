...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const amanha = new Date(now + 86400000).toISOString().split('T')[0];
    ...
    for (const a of ags.filter(a => a.data === amanha && a.status !== 'cancelado')) {
      await pushToUsuario(a.usuario_id, {
        ...
        body: `Você tem ${a.servico} amanhã às ${a.horario} com ${barNome(a.barbeiro_id)}. Até lá! ✂️`,
        tag: `lembrete-${a.id}`,
      });
      lembretes++;
    }
