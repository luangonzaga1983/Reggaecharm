...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (purpose === 'agendamento') {
      ...
      // Crédito já abatido no agendamento: PIX cobra só o restante.
      const credUsado = Number(ag.credito_usado ?? 0);
      const aPagar = Math.round((ag.valor - credUsado) * 100) / 100;
      if (!aPagar || aPagar < 1) return err('Valor inválido para cobrança');
