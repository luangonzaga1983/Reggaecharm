...
import { getSession, getLiveSession } from '@/lib/auth';
...
export async function GET() {
  try {
    ...
    // `comissoes` (% por barbeiro) é dado financeiro sensível — só o dono vê.
    // Todos os outros (inclusive não autenticados na landing) recebem sem isso.
    const session = await getSession();
    if (session?.role !== 'dono') {
      const { comissoes, ...publico } = config;
      return NextResponse.json({ config: publico });
    }
